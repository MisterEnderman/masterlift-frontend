...
function Carousel({ plans, radius = 2 }) {
    // אופציה 1: סינון פרויקטים שאין להם תמונה בכלל
    const validPlans = plans.filter(proj => proj.imageUrl);
    const count = validPlans.length;

    return validPlans.map((project, i) => {
...
export default function ShowcasePage() {
    ...
    const MY_EMAIL = "itamarbenamidaniel@gmail.com";

    useEffect(() => {
        const fetchPlans = async () => {
            try {
                // apiRequest כבר מחזירה את ה-data המפוענח (response.json())
            ...
            <Canvas camera={{ position: [0, 0, 100], fov: 15 }}>
                ...
                <ScrollControls pages={4} infinite>
                    <Rig rotation={[0, 0, 0.15]}>
                        <Carousel plans={myPlans} radius={1.8} />
                    </Rig>
